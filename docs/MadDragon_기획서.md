...
- `CampaignHubScreen`으로 보유 자원, 저장 자원, 예상 약탈 손실을 첫 화면에서 확인하고 공격 준비 또는 기지 정비로 이동할 수 있게 했다.
- `BaseManagementScreen`으로 본부 레벨, 저장 한도, 저장 골드, 예상 손실을 표시하고 `Collect` 버튼으로 미수거 자원을 보유 자원으로 옮길 수 있게 했다.
- `TestBootstrap`에 자원 생산 틱, 현재 약탈 예측, 허브/기지/기존 공격 준비 화면 전환을 연결해 자동 생산 자원을 자주 챙기는 루프를 프로토타입 화면에 반영했다.
...
- 테스트 편의를 위해 공격 병력이 비어 있을 때 `Start`를 누르면 기본 기사 3명을 자동 편성하고 전투를 시작한다.
