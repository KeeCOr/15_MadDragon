...
- 건물 크기: Toony 건물 모델은 렌더러 bounds를 기준으로 게임플레이 건물의 바닥 크기에 맞춰 자동 스케일링하고, 하단을 지면에 정렬한다.
- FOW 연출: 공격 모드에서는 미탐색 지역을 짙은 어두운 안개로 덮고, 탐색했지만 현재 시야 밖인 지역은 옅은 안개로 남기는 전장 오버레이를 사용한다.
